...
<template>
    <section id="contacts" class="navigation" :class="{ 'cookie-accepted': isCookieAccepted }">
        <div class="contacts">
            <h5>Контакты</h5>
            <ul>
        ...
        <transition name="map-fade-scale">
            <yandex-map
...
.map-fade-scale-enter-active,
.map-fade-scale-leave-active {
  transition: all 0.5s ease;
}

.map-fade-scale-enter-from,
.map-fade-scale-leave-to {
  opacity: 0;
  transform: scale(0.95);
}

.map-fade-scale-enter-to,
.map-fade-scale-leave-from {
  opacity: 1;
  transform: scale(1);
}
